...
\section{The prior}
\subsection{Theory}
\question \emph{1.} Choosing a gaussian is a sensible thing to do because it makes calculation much easier, Gaussians are quite easily multiplied and integrated which are things you often have to do in machine learning contexts e.g. multiplying to compute a joint distribution or integrating to marginalise out a variable. Choosing a Gaussian likelihood also fits with our model because we often assume errors observation errors to be independent and identically distributed which due to the Central Limit Theorem (which states the distribution of the sum of a large enough number of independent, identically distributed variables will be approximately normally distributed) means our model ends up being a Gaussian.
...
\subsubsection{Linear regression}
\question Figure \ref{fig:prior} represents our prior assumption over $\mat{W}$. Our prior assumption states that the distribution over $\mat{W}$ be a Gaussian with a mean of $0$ and the identity matrix as its covariance. As discussed in question 3 the likelihood is a Gaussian, therefore choosing a Gaussian prior allows us to obtain a Gaussian posterior.

In Figures \ref{fig:1-s}, \ref{fig:2-s} and \ref{fig:15-s} we can see how our assumptions about $\mat{W}$ change as we observe more data. Making the distribution over $\mat{W}$ a posterior or our updated belief. These are paired with samples from our posterior that allow us to plot some sample functions. It is observable that the more data we observe, the more the uncertainty in our posterior reduces giving us a posterior that approaches the values of the actual parameters $\mat{W}$ displayed as a white cross, and increasingly accurate sample functions.
\begin{figure}[H]
	\centering
...
\end{figure} \\
\subsubsection{Non-parametric regression}
\question The covariance function, or kernel, encodes all the assumptions about the form of function we are modelling, and often represents some form of “distance” or similarity between the data.

Figure \ref{fig:GP-p} shown represent samples from our GP-Prior created with a squared exponential covariance function. The length scale allows us to put constraints on how smooth the functions are. Small lengthscale values characterize functions that change quickly, whereas larger values characterize functions that change slowly. The three figures represent our GP-prior with different lengthscale values. We can observe that the high lengthscale value in Figure \ref{fig:10.l} gives us smooth functions while the lower lengthscale value in \ref{fig:0.1l} gives us a functions with rapidly changing values.
...
\begin{figure}[H]
	...
	\label{fig:GP-p}
\end{figure}
\question Figure \ref{fig:samples} shows the predictive posterior samples, or our updated prior after having observed data. While our prior showed functions that corresponded to our general prior assumptions, the posterior shows only the set of constrained functions that pass by the observed data points.

As the figure shows, this behavior is not desirable as is can lead to overfitting. Considering that there can be noise in the data, such as in Figure \ref{fig:samples}, overfitting will not enable us to find the underlying function, in this case a sine wave. We can observe that our uncertainty drops as we observe data but rises again as we get away from the observed data. This is due to our squared exponential covariance matrix that encodes an assumption in the form of  the functions such that if two points $x_1$ and $x_2$ are close together, we expect their corresponding $y$ values to be close too. Therefore the further away a new $x$ value is from the $x$ values of observed data points, the more uncertain we are about its $y$ value.

Adding a diagonal covariance matrix to the covariance function encodes uncertainty in our model. As we can see in Figure \ref{fig:variancewithdiag}, there is a certain degree of error encoded in each data point observed, meaning that the underlying function does not pass through every observed data point, allowing us to obtain a smoother line that resembles far more the underlying sine function.
\begin{figure}[H]
	\centering
