...
\end{align*}
By replacing these Gaussians with their probability density function and combining we can form a single exponential, by then completing the square with regard to $x$ we can integrate $x$ out to form a new exponent for the marginal with mean $0$ and covariance $\mat{W}\mat{W}^T + \sigma^2\mat{I}$ thus making our marginal distribution:
\begin{align*}
